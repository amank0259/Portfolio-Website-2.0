...
const SkillSection = ({ skillsData, theme }) => {
    const getSkillIconSrc = (theme, skill) => {
        if (
            theme === "dark" &&
    ...
    const getSkillColor = (theme, skill) => {
        if (
            theme === "dark" &&
            (skill.title.includes("Next") || skill.title.includes("Express"))
        ) {
    ...
    return (
        <article
            className={`h-auto rounded-2xl p-16 pt-32 grid grid-cols-3 gap-10 relative z-10 max-lg:w-full max-lg:grid-cols-2 max-lg:p-8 max-lg:pt-32 ${theme === "dark"
        ...
        >
            ...
            {skillsData[0].skills.map((skill, index) => (
                <div
                    key={index}
                    className={`skill-item cursor-pointer flex flex-col gap-8 rounded-2xl p-8 border-solid border-[0.25rem] text-center max-lg:items-center ${theme === "dark" ? "bg-[--darkblue]" : "bg-[--icewhite]"
                        }`}
                ...
                >
                    <img
                        ...
                        className="h-[10rem]"
                    />
                    <h3 className="max-lg:text-[1.5rem] min-[1024px]:hidden">
                        {skill.title}
                    </h3>
                </div>
            ))}
